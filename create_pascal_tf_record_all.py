...
def dict_to_tf_example(image_path,
    ...
    example = tf.train.Example(features=tf.train.Features(feature={
        ...
        # 'label/encoded': tf.train.Feature(float_list=tf.train.FloatList(value=label)),
...
def create_tf_record(output_filename,
    ...
    for idx, example in enumerate(examples):
        if idx % 10 == 0:
            ...
            tf.logging.info('On image %d of %d, elapsed: %03d:%02d',
                            idx,
                            len(examples),
                            np.floor(elapsed / 60),
                            elapsed % 60)
...
def main(unused_argv):
    ...
    # with open(os.path.join(FLAGS.data_dir, 'val.txt'), 'w') as f:
    #  f.write(','.join(val_examples))

    train_output_path = os.path.join(FLAGS.output_path, 'voc_train_all.record')
